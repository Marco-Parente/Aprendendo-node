...
cli.responders.help = function(){
    var commands = {
        ...
        'list users' : 'Show a list with all the registered (undeleted) users in the system',
        ...
        'more log info --{fileName}' : 'Show details of a specified logs file'
    };
    ...
    // Show each command followed by its explatnation in white and yellow respectively
...
cli.centered = function(str){
    ...
    // Put in left padded spaces before the stirng itself
    var line = '';
        ...
        if(!err && checkIds && checkIds.length > 0){
            ...
            checkIds.forEach(function(checkId){
                _data.read('checks',checkId,function(err,checkData){
                    ...
                    var stateOrUnknown = typeof(checkData.state) == 'string' ? checkData.state : 'unknwon';
...
cli.responders.listLogs = function(){
    ...
    ls.stdout.on('data',function(dataObj){
        // Explode into separeted lines
        var dataStr = dataObj.toString();
        logFileNames = dataStr.split('\n');
        cli.verticalSpace();
